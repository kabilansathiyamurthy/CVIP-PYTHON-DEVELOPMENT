...
words=['apple','pear','pair','mango','car','bike','cat','dog','horse','fan','air','blue','red','black','grey',
       ...
       'duck','horn','crown','fish','fly','plain','laptop','computer','mouse','keyboard','monitor','python','code','complete']
...
def time():
    ...
    if(timeleft>0):
        ...
        if matched>=35 and matched<=40:
            feedback.configure(text="Your WPM is average")            #feedback on last
        elif matched>=65:
            feedback.configure(text="Your WPM is above average")
        else:
            feedback.configure(text="Your WPM is below average")
...
#Topic
h1=Label(root, text=" Typing Speed Test ", bg="#001D3D", fg="#FFC300" ,font="comicsanms 24 bold", anchor="center")
h1.pack(pady=10)
...
label=Label(root, text="Please Ensure the Caps And Small Letters", bg="#001D3D", fg="grey", font="comicsanms 10 italic")
label.place(x=55,y=300) #instruction 1 for user below entry label at starting of game
